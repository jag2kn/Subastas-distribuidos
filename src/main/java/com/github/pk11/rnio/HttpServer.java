...
public class HttpServer implements Runnable {
	...
	public final Reactor reactor = Reactors.reactor()
																	.env(new Environment())
    ...
    public static class DemoHandler implements Handler {
        ...
        public void handle(HTTPRequest request, HTTPResponse response) {
            ...
            String []parametros = request.location.split("/");
            ...
            if (parametros.length>=2){
	            switch(parametros[1]){
	            	case "login":
	            		respuesta = "Ingresando al sistema";
	            		if (parametros.length>=3){
		            		nombres += parametros[2]+", ";
	            		}else{
	            			respuesta = "Es necesario un nombre para el login";
	            		}
	            		break;
	            	case "estado":
	            		respuesta = "La mayor oferta es: "+valor+" hecha por "+ofertanteMayor;
	            		break;
	            	case "ofertar":
	            		if (parametros.length>=4){
	            			int ofertaNueva=Integer.parseInt(parametros[2]);
	            			if (valor<ofertaNueva){
	            				valor = ofertaNueva;
	            				ofertanteMayor = parametros[3];
	            				respuesta = "Nueva oferta realizada "+valor+" por "+ofertanteMayor;
	            			}else{
	            				respuesta = "Oferta menor a la actual<br>";
	            				respuesta = "La mayor oferta es: "+valor+" hecha por "+ofertanteMayor;
	            			}
	            		}else{
	            			respuesta = "Parametros incorrectos en la oferta";
	            		}
	            		break;
	            	default:
	            		respuesta = "Respuesta sin procesar ["+parametros[0]+","+parametros[1]+"]";
	            		break;
	            }
            	
            }

            response.content("<h2>"+request.location+"</h2>Respuesta: "+respuesta+"<br>Nombres: "+nombres).end();
        }
    }
